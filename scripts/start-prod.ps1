...
foreach ($command in @('docker', 'npm', 'mvn', 'java')) {
...
Write-Host '========================================'
Write-Host 'AetherLearn production startup'
Write-Host 'Spring Boot: http://127.0.0.1:8080'
Write-Host 'Vue static assets: served by Spring Boot'
...
Write-Host 'Building Vue production assets...'
...
$jar = Get-ChildItem -Path (Join-Path $backendDir 'target') -Filter '*.jar' | Where-Object { $_.Name -notmatch 'sources|javadoc' } | Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (-not $jar) {
    throw 'No Spring Boot executable JAR was found.'
}

Write-Host 'Starting Spring Boot production service...'
$backendEnv = @(
    ...
    "java -jar '$(ConvertTo-PSSingleQuoted $jar.Path)'"
) -join '; '
...
Write-Host 'Project startup completed. Spring Boot is available at http://127.0.0.1:8080.'
